...
const Accordion = () => {
    ...
    useEffect( () => {
        axios.get(`http://localhost:8080/categories`,{  headers: {"Authorization" : `Bearer ${jwt}`} })
          .then(resp =>  {
    ...
    console.log("cat" +JSON.stringify(category))
       ...
       const retrieveUrl = (videoName,courseID) => {
         ...
         const i =category.find((video)=>video.videoName===videoName && video.course.courseID === courseID);
        const url= i.url;
...
<div>
    ...
    <div className="sidenava" style={{ position: "relative", zIndex: 40 }}>
            ...
            {activeIndex === index && (
              <div className="dropdown-container">
                {item.videos.map((link, i) => (
                  <a href="#!" key={i} onClick={() => {


                
                    playVideo(link.url)
            }}>
                    {link.videoName}<i className="fa-regular fa-circle-play"></i>
                  </a>
                ))}
              </div>
            )}
